...
def test_clean_quantity_triggers_value_error(monkeypatch):
    # Força erro no float
    monkeypatch.setattr("builtins.float", lambda x: (_ for _ in ()).throw(ValueError("mock")))
...
def test_parse_category_table():
    ...
    result = helpers.parse_category_table(table, 2022, "Categoria", "Produto", "Quantidade")
