...
def test_processing_route_invalid_type(access_token):
    ...
    """
    response = client.get(
        "/processing/?year=2022&type=invalid_type",
...
def test_processing_route_fallback_failure(mock_no_processing_data, access_token):
    ...
    response = client.get(
        f"/processing/?year=2022&type={valid_type}",
        headers={"Authorization": f"Bearer {access_token}"},
    )
    assert response.status_code == 503
    assert (
        response.json()["detail"]
        == "Unable to fetch processing data from Embrapa or fallback."
    )
...
def test_processing_route_unmapped_enum_value(access_token, monkeypatch):
    ...
    monkeypatch.setitem(
        processing.GRAPE_TYPE_TO_SUBOPT, GrapeTypeEnum.sem_classificacao, None
    )

    response = client.get(
        f"/processing/?year=2022&type={GrapeTypeEnum.sem_classificacao.value}",
        headers={"Authorization": f"Bearer {access_token}"},
    )
